...
class SlideWindow:
    ...
    def curve(self, img) :
        curve_img = img[250:390, 220:400] # y x
        stopline_count = cv2.countNonZero(curve_img)
        ratio = stopline_count/(140*180)
        print(f"ratio: {ratio}")

        return curve_img, ratio
