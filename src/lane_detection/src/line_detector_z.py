...
from math import *
from slide_window_z import SlideWindow
...
class LineDetector() :
    ...
    def __init__(self) :
        ...
        self.ctrl_cmd_pub = rospy.Publisher('/z_ctrl_cmd', CtrlCmd, queue_size=1)
        self.z_curve_pub = rospy.Publisher('/z_curve', Int16MultiArray, queue_size=1)
    ...
    def imageCallback(self, _data) :
        if (self.original_longitude  <= 1 and self.original_latitude <=  1):
            ...
            if self.ratio > 0.1 and self.first_curve == 1:
                self.second_curve = 1

            # print("first curve", self.first_curve)
            # print("second_curve", self.second_curve)
            ...
            self.gap = 360 - self.slide_x_location 
    ...
    def publishCtrlCmd(self, motor_msg, servo_msg, brake_msg):
        ...
        self.ctrl_cmd_msg.steering = self.steering_angle
        self.ctrl_cmd_msg.brake = brake_msg
        self.ctrl_cmd_pub.publish(self.ctrl_cmd_msg)
    
    # def publishZcurve(self):
    #     self.data = self.first_curve, self.second_curve
    #     self.z_curve_pub.publish(self.data)
    
    def publishZcurve(self):
        z_curve_msg = Int16MultiArray()
        z_curve_msg.data = [self.first_curve, self.second_curve]
        self.z_curve_pub.publish(z_curve_msg)
